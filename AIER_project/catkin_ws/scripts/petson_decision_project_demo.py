...
#result_grid = model.tune(data='/home/orin/scotty-4/scotty-4/data.yaml', space={'lr0':tune.uniform(1e-5, 1e-1)}, epochs=10, use_ray = True)
...
def run(cmd):
    ...
    p.wait()
    # p.kill()
def query_huggingface_model(model_name, prompt, api_token, wait_for_model=True):
...
class VoiceRecognition:
    def __init__(self, *args):
        ...
        # self.HF_API_TOKEN = "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" # Taewoo's 
...
class image_converter:
    ...
    def callback(self, data):
        ...
        track_results = model.predict(img, conf=0.5, verbose=False, classes=[0], half=True) # 0.35
        have_results = track_results[0] if track_results else None

        if have_results:
            # Plot the image with detection
            detected_im_array = track_results[0].plot(conf=True, boxes=True, probs=True)
...
def audio_main(*args):
    # ARGS
    TTS_pub = rospy.Publisher('tts_speak', String, queue_size=1)
    TTS_ON = True
    global last_message
    # Mode, Path, textfile
    try:
    ...
    while not rospy.is_shutdown():
        ...
        if leave:
          ...
          if os.path.exists(textfile):
            ...
            if s is not None:
              try:
                print(f'Content of the text file: {s[0]}\n')
              except:
                continue

              # Prompt: recognize the command
              prompt = f"What is the intention of the given text? Select one from the options.\nText: {s[0]}\nOptions: 'Turn left(Direction)', 'Turn right(Direction)', 'Move forward(Direction)', 'Go back(Direction)', 'Return to home(Direction)', 'Question'\nDo not contain Explanation and emoji in the answer.\nAnswer:"
              ...
              if not ('question' in result.lower()):
                # Do command
                ...
                # Filtering
                # Prompt: QA
                print('Question in QA else:', s[0])
                #prompt_for_answer = f"You are a moving robot that helps people. Answer to the given text within only one sentence and with words only: {s[0]}?"

                prompt_for_answer = f"Your name is 'Petson' and you are a moving robot that helps people. Act as a 'Petson'. Generate an answer to the given text within only one sentence and with words only: {s[0]}"
                
                # prompt_for_answer = f"Please respond to the last sentence. You are a PET named 'PETSON'. I would appreciate a concise answer. Do not include any emoticons.\n{s[0]}"
...
if __name__ == "__main__":
    ...
    rospy.Subscriber('tts_speak', String, TTS_callback, queue_size=1)
    ...
    Angle = 60
    start, linear, angular = False, default_speed, default_turn #False, 0.3, 1.0
        ...
        while not rospy.is_shutdown():
            ...
            if recognized_direction is not None:
                ...
                if "move forward" in recognized_direction:
                    # Forward
                    twist.linear.x = linear #1 #원래는 linear = 0.3
                    twist.angular.z = 0
                elif "turn left" in recognized_direction:
                    # Turn left
                    twist.linear.x = 0
                    twist.angular.z = angular #1.5 #원래는 angular = 1
