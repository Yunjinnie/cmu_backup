...
import os, sys, tty, time, select, random, termios, requests, collections, multiprocessing
import wave, pyaudio, webrtcvad
import tf, rospy, roslib, actionlib
import cv2
...
# Refactor: Move utils to petson_decision_utils.py
from petson_decision_utils import {
        run, record_to_file, normalize, query_huggingface_model,
        movebase_client, convert_arr_to_ros_msg,
        moveBindings, speedBindings,
        getKey, vels, move_bigger,
        }
...
YOLO_PATH='/home/orin/yolov8_rs/yolov8s.pt' #'/home/orin/catkin_ws/src/best4.pt'
          ...
          if os.path.exists(textfile):
            ...
            if s[0] is not None:
              ...
              if not ('question' in result):
                ...
                # Filtering
                # Prompt: QA
                prompt_for_answer = f"Your name is 'Petson' and you are a moving robot that helps people. Act as a 'Petson'. Answer to the given text within two sentences:{s[0]}"
...
if __name__ == "__main__":
    ...
    avoid_distance = 0.4 #min(2*speed, 0.3) #0.2
    Angle = 60
    ...
    TTS_pub = rospy.Publisher('speak', String, queue_size=1)
        ...
        while not rospy.is_shutdown():
            ...
            if recognized_direction is not None:
                ...
                if "move forward" in recognized_direction:
                    # Forward
                    twist.linear.x = 1 #linear#1 #원래는 linear = 0.3
                    twist.angular.z = 0
                elif "turn left" in recognized_direction:
                    # Turn left
                    twist.linear.x = 0
                    twist.angular.z = 1.5#angular#1.5 #원래는 angular = 1
