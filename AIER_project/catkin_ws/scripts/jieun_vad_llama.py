...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if Mode == "asr_en":
            ...
            print('running pushtotalk...')
            run("googlesamples-assistant-pushtotalk --project-id iitp-46605 --device-model-id iitp-46605-jetson-zmlp68 --once")

            # try:
            #     subprocess.run("googlesamples-assistant-pushtotalk --project-id iitp-46605 --device-model-id iitp-46605-jetson-zmlp68")
            # except Exception as e:
            #     print("Error running pushtalk.py:", e)
            ...
            #         s=f.readlines()
            #     if s is not None:
            #         print('recoreded content:', s[0])
                    
            #         #songmu
            #         # prompt = f"After reading the following sentence, choose one of the following options: 'Turn left', 'Turn right', 'Move forward', 'Step backward'. {s[0]}"
            #         # prompt = f"What is the intension of the given text. \nText: "
            #         prompt = f"What is the intention of the given text? Select one from the options.\nText: {s[0]}\nOptions: 'Turn left', 'Turn right', 'Move forward', 'Step backward\nAnswer:"
            #         result = query_huggingface_model(model_name, prompt, api_token)
            #         print(result)
