...
YOLO_PATH='/home/orin/yolov8_rs/yolov8s.pt' # n # m
...
def moving(pub, turn):
    ...
    for i in range(3):
        pub.publish(twist)
    twist.linear.x = -1
    twist.angular.z = - turn * 1.57 # radian
    pub.publish(twist)
...
if __name__ == "__main__":
    ...
    default_turn = turn
    avoid_distance = 0.3 #0.4 # min(2*speed, 0.3) #0.2 #0.2
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            try:
                ...
                # map: original position, base: present position
                #print(trans)
                #print(rot)
                rospy.loginfo('---------')
            ...
            if safe_drive > np.ceil(int(RATE/2)):
                avoid = 0

            if safe_drive % 2 == 0:
                keyid += 1
                key = move_list[keyid % 4]
