...
class MyDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.data_partition == "train":
            (clip_video_path_1, clip_video_path_2), (clip_audio_path_1, clip_audio_path_2), category_idx = self.samples[idx]
            ...
            return {
                ...
                'class': torch.tensor(category_idx, dtype=torch.long)
            }

        # Get item from test dataset
        elif self.data_partition == "test":
            full_video_path, full_audio_path, annotations = self.samples[idx]
            ...
            for ann in annotations:
                ...
                # class
                clip_class = self.category_to_idx[ann['category']]
                segments_class.append(torch.tensor(clip_class))
            ...
            return {
                ...
                'class': torch.stack(segments_class)
            }
    ...
    def process_audio(self, audio_data, sr):
        # Calculate MFCCs
        mfcc = librosa.feature.mfcc(y=audio_data, sr=sr, n_mfcc=13)  # [n_mfcc, T]

        # Normalize MFCCs
        mfcc = librosa.util.normalize(mfcc)

        # Convert to PIL Image (single channel)
        mfcc_image = Image.fromarray(mfcc).convert('L')

        # Apply preprocessing transformations
        mfcc_processed = self.preprocessing_func['audio'](mfcc_image) # torch [1, 224, 224]
