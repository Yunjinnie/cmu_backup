...
class Trainer(object):
    ...
    def _train(self, dataloader):
        ...
        for idx, batch in enumerate(dataloader):
            ...
            frames = frames[:, 0, :, :, :] # torch.Size([B, C, H, W])
            mfccs = batch["mfcc_data"] # torch.Size([B, T, C, H, W])
            # Selecting the first frame |
            mfccs = mfccs[:, 0, :, :, :] # torch.Size([B, C, H, W])
            label = batch["class"] # torch.Size([B,])
            ...
            num_correct += int((torch.argmax(outputs[0], axis=1) == label).sum())
