        ...
        with torch.inference_mode():
            for idx, batch in enumerate(dataloader):
                ...
                for i in range(T):
                    ...
                    mfcc = mfccs[i]
                    mfcc_label = mfcc_labels[0][i]

                    outputs = self.model(video, mfcc) # outputs = tuple(weigted_v_softmax, weigted_a_softmax)
                    v_loss = self.criterion(outputs[0], video_label)
                    a_loss = self.criterion(outputs[1], mfcc_label)
                    v_a_loss = v_loss + a_loss
                        
                    current_video_correct += int((torch.argmax(outputs[0], axis=1) == video_label).sum())
                    current_mfcc_correct += int((torch.argmax(outputs[1], axis=1) == mfcc_labels).sum())
