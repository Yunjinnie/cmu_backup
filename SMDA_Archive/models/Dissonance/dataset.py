...
class MyDataset(torch.utils.data.Dataset):
            ...
            for category in tqdm(os.listdir(self.data_path), desc='Loading categories'):
                ...
                for person_id in tqdm(os.listdir(clip_video_path), desc=f'Processing clips', leave=False):
                    ...
                    for file_id in os.listdir(person_clips_path):
                        ...
                        if min_length < 2:
                            continue
                            print(f"min_length is zero.")
                            print(f"clips_files: {len(clips_files)}, audio_files: {len(audio_files)}")
                            print(f"clip path: {file_clips_path}")
                            print(f"listdir(clips): {os.listdir(file_clips_path)}")
                            print(f"audio path: {file_audio_path}")
                            print(f"listdir(audio): {os.listdir(file_audio_path)}")
    ...
    def __getitem__(self, idx):
        ...
        if self.data_partition == "train":
            (clip_video_path_1, clip_video_path_2), video_class, (clip_audio_path_1, clip_audio_path_2), audio_class = self.samples[idx]
            ...
            #return {
            #    'video_data': torch.stack([video_frames_1, video_frames_2], dim=0) if (len(video_frames_1)+len(video_frames_2) > 0) else torch.tensor([]),
            ...
            #}
            return {
                'video_data': video_frames_1.unsqueeze(0) if (len(video_frames_1) > 0) else torch.tensor([]),
                'video_class': torch.tensor(video_class, dtype=torch.long),
                'mfcc_data': mfcc_features_1.unsqueeze(0),
                'mfcc_class': torch.tensor(audio_class, dtype=torch.long)
            }

        # Get item test dataset
        elif self.data_partition == "test":
            full_video_path, video_class, full_audio_path, audio_class, annotations = self.samples[idx]
    ...
    def process_video(self, video_path):
        #print(f"**processs_video({video_path})**")
    ...
    def process_audio(self, audio_data, sr):
        ...
        mfcc = self.preprocessing_func['audio'](mfcc) # torch [1, n_mfcc=224, T=224]
        return mfcc