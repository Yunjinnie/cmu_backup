...
class Trainer(object):
    ...
    def _train(self, dataloader):
        ...
        for idx, batch in enumerate(dataloader):
            if len(batch['video_data']) == 0: continue
            ...
            with torch.cuda.amp.autocast():
                outputs = self.model(frames, mfccs, return_feats=True) # outputs = tuple(weigted_v_softmax, weigted_a_softmax)
                #v_loss = self.criterion(outputs[0], frames_label)
                #a_loss = self.criterion(outputs[1], mfccs_label)
                #v_a_loss = v_loss + a_loss
                v_a_loss = self.criterion(outputs[0], outputs[1], outputs[2], outputs[3], (frames_label, mfccs_label), args.hyper_param)
        ...
        with torch.inference_mode():
            for idx, batch in enumerate(dataloader):
                ...
                for i in range(1,T):
                    ...
                    v_a_loss = self.criterion(outputs[0], outputs[1], None, None, (frames_label, mfccs_label), args.hyper_param)
